        ...
        if type(thread) == discord.Thread:
            ...
            async for message in channel.history():
                if str(thread.id) in message.embeds[0].description:
                    ...
                    new_embed = discord.Embed(title=f"{old_title} - CLOSED", description=old_description)
                    await channel.send(embed=new_embed)
                else:
                    await ctx.send("Could not find bugs in #dev-bugs! Please edit manually.")
        ...
        if type(thread) == discord.Thread:
            ...
            async for message in channel.history():
                if str(thread.id) in message.embeds[0].description:
                    ...
                    old_description = message.embeds[0].description
                    await message.delete()
                    new_embed = discord.Embed(title=title, description=old_description)
                    await channel.send(embed=new_embed)
